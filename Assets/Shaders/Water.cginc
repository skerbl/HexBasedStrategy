...
float Waves (float2 worldXZ, sampler2D noiseTex) 
{
	...
	// First factor controls size of waves. Second factor controls intensity.
	float blendWave = sin(
		(worldXZ.x + worldXZ.y) * 0.05 +
		(noise1.y + noise2.z) + _Time.y) * 0.5;
	blendWave *= blendWave;
	...
	// Smoothstep parameters control intensity of the overall effect.
	return smoothstep(0.75, 2, waves);
}

float River(float2 riverUV, sampler2D noiseTex) 
{
	// Slide two noise samples in opposite direction, one faster than the other.
	// This avoids obvious repetitions in the pattern.
	float2 uv = riverUV;
